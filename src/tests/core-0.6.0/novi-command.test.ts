...
describe('parseNoviCommand', () => {
  it('returns handled: false for non-#novi lines', () => {
    ...
    expect(parseNoviCommand('echo #novi myfile.md')).toEqual({ handled: false });
    expect(parseNoviCommand('foo #novi')).toEqual({ handled: false });
  });
  ...
  it('returns kind: open with path for "#novi <path>"', () => {
    ...
    expect(parseNoviCommand('#novi myfile.md')).toEqual({ handled: true, kind: 'open', path: 'myfile.md' });
  });
