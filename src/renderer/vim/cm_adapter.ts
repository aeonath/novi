...
class CMAdapter {
  ...
  handleCursorChange = (e) => {
    ...
    // Safety check: model may be null during tab switches or disposal
    if (!model) {
      return;
    }
  ...
  getLine(line) {
    ...
    return model.getLineContent(line + 1);
  }
  ...
  getRange(start, end) {
    ...
    return model.getValueInRange(Range.fromPositions(p1, p2));
  }
  ...
  getSelection() {
    ...
    if (!model) return "";
    const list: string[] = [];
    this.editor.getSelections().forEach((sel) => {
      list.push(model.getValueInRange(sel));
    });
    return list.join("\n");
  }
  ...
  setCursor(line, ch) {
    ...
    const monacoPos = model.validatePosition(toMonacoPos(pos));
